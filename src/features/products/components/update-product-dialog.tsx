...
import type { UpdateProductDTO } from '@/core/use-cases/update-product.use-case'
import { useEffect } from 'react'

type UpdateProductDialogProps = {
  open: boolean
  onClose: () => void
  handleSubmit: (data: UpdateProductDTO) => Promise<void>
...
export const UpdateProductDialog: React.FunctionComponent<UpdateProductDialogProps> = props => {
  ...
  async function handleSubmit(data: UpdateProductSchemaOutput) {
    if (!props.product) return

    await props.handleSubmit({ id: props.product?.id, ...data})
    props.onClose()
    form.reset()
  }

  useEffect(() => {
    function updateFields(product: Product) {
      form.setValue('name', product.name)
      form.setValue('value', product.value)
      form.setValue('composition', product.composition.map(i => ({
        rawMaterialId: i.materialId,
        requiredQuantity: i.quantity,
      })))
    }
    if (props.product)
      updateFields(props.product)
  }, [props.product])
...
type UpdateProductSchemaOutput = z.output<typeof updateProductSchema>
